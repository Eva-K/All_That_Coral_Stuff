...
FNGBR_Waves_val<- mutate(FNGBR_Waves_val, Latitude=P_Lat)
FNGBR_Waves_val<- mutate(FNGBR_Waves_val, Longitude=P_Long)

FNGBR_Val <- FNGBR_Waves_val%>%
  select("Latitude":"Longitude")

# try to extract predictions for the val points - Ha Ha
# This shouldn't work as they weren't used for the modelling

#FNGBR_Valpreds <- merge(FNGBR_Val, FNGBR_ALL_simple, by=c("Longitude","Latitude"),all.x=TRUE, all.y=FALSE) 

FNGBR_Valpreds <- left_join(FNGBR_Val, FNGBR_ALL_simple, by = "Longitude")

FNGBR_Valpreds <-mutate(FNGBR_Valpreds, "Latitude" = Latitude.x)

FNGBR_Valpreds <- left_join(FNGBR_Valpreds, FNGBR_ALL_simple)

FNGBR_Valpreds <-na.omit(FNGBR_Valpreds)
#######
